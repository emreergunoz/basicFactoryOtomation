...
<body style="background:#343a40;"> 
	...
	<?php if (($_SESSION['yetki']==1)) {?>
        <?php if(isset($_GET['id'])){
            ...
            $id=$_GET['id'] ;
            $tur=$db->query("SELECT * FROM tbl_hazir_isler WHERE is_id=$id")->fetch(PDO::FETCH_ASSOC);
	...
	<div class="container col-md-8 text-light">
		...
		<form action="islem.php" method="post">
			<input type="hidden" name="islem" value="is_guncelle">
			<input type="hidden" name="id" value="<?$tur['is_id';]?>">
			<div class="input-goup mb-3 mt-5">
				<div class="input-group mb-3">
					<span class="input-group-text" id="bassic-addon1">İş Adı</span>
					<input class="form-control" name="isad" value="<?=$tur['is_ad'];?>" required="">
				</div>
			</div>
			...
			<div class="input-goup mb-3">
				<div class="input-group mb-3">
					...
					<input class="form-control" name="issure" value="<?=$tur['id_sure'];?>" required="">
				</div>
			</div>
